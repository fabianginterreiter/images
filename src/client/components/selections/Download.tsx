  ...
  public render() {
    return (
      <div onClick={this.toggle.bind(this)} className="right">
        <span><i className="fa fa-download" aria-hidden="true" /> <span className="min500"> {t("selection.download")}</span></span>
        <Dropdown open={this.state.visible} onCancel={this.close.bind(this)}>
          <ul className="options">
            <li><a onClick={() => this.handleDownload()}>{t("selection.downloadOriginalSize")}</a></li>
